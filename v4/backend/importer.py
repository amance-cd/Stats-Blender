...
def spotify_api_and_insert(data):
    ...
    yield f"Found {len(unique_ids)} unique songs to research on Spotify"
    ...
    for i in range(0, len(unique_ids), 50):
        ...
        if progress >= j:
            yield f"Spotify history progression : {progress}%"
            j = progress + 1
    ...
    #Insert data
    yield "Inserting data into database..."
...
def deezer_api_and_insert(data):
    ...
    unique_isrcs = data['isrc'].dropna().unique().tolist()
    isrcs_to_search = [isrc for isrc in unique_isrcs if isrc not in db_isrc_info or db_isrc_info[isrc] != "album"]

    yield f"Found {len(isrcs_to_search)} tracks to research on Spotify"
    ...
    yield "Updating database..."
    for isrc, track in isrc_winners.items():
        spotify_id_winner = track['external_urls']['spotify'][31:]
        artist = track['artists'][0]
        conn.execute("INSERT OR IGNORE INTO artists (id, name) VALUES (?, ?)", (artist['id'], artist['name']))
        album = track['album']
        conn.execute("INSERT OR IGNORE INTO albums (id, name, artist_id, album_type) VALUES (?, ?, ?, ?)", (album['id'], album['name'], artist['id'], album.get('album_type', 'unknown')))
        if isrc in db_isrc_info:
...
def run_import(db_name, source_folder, source_type=None):
    ...
    yield f"Starting import for {db_name}..."
    
    # Process Spotify
    spotify_data = load_spotify_history(source_folder)
    if not spotify_data.empty:
        yield from spotify_api_and_insert(spotify_data)
    else:
        print("No Spotify JSON history files found.")

    # Process Deezer
    import glob
    deezer_files = glob.glob(os.path.join(source_folder, "*.xlsx"))
    if deezer_files:
        # Pass the first file path found
        deezer_data = load_deezer_history(deezer_files[0])
        if not deezer_data.empty:
            yield from deezer_api_and_insert(deezer_data)
    else:
        print("No Deezer XLSX files found.")
            
    yield "Cleaning up duplicates..."
    deduplicate_albums()
    yield "SUCCESS"
...
if __name__ == "__main__":
    ...
    run_import("spotify_history.db", spotify_folder, "spotify")
    
    end_time = time.time()
    minutes = int((end_time - start_time) // 60)
    secondes = int((end_time - start_time) % 60)
    print(f"\n--- TOTAL IMPORT TIME: {minutes}m {secondes}s ---")
